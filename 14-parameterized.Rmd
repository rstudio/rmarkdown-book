...
Any R expressions included within the parameters are executed before any code in the document, therefore any package dependencies must explicitly stated using the `package::function` notation (e.g., `!r lubridate::today()`), even if the package is loaded later in the Rmd document.
...
Parameters can also be used to control the behavior of **knitr**. For example, we may want to change the settings whether to display the code using the chunk option `echo` globally in the document:
...
Parameterized reports are supported by the publishing platform RStudio Connect (https://www.rstudio.com/products/connect/). If you publish a parameterized report to an RStudio Connect server, you will be able to compile reports by interactively choosing different parameter values on the server, and easily store/navigate through different reports built previously.
