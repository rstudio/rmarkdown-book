...
The **learnr** package [@R-learnr] makes it easy to turn any R Markdown document into an interactive tutorial. Tutorials consist of content along with interactive components for checking and reinforcing understanding. Tutorials can include any or all of the following:
...
To create a hint based on custom Markdown content, add a `<div>` tag with an `id` attribue that marks it as hint for your exercise (e.g., `filter-hint`). For example:
...
The functions `question()` and `answer()` have several other arguments for more features that allow you to customize the questions and answers, such as custom error messages when the user's answer is wrong, allowing users to retry a question, multipe-choice questions, and multiple questions in a group. See their help pages in R for more information.
