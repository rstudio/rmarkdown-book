...
This book itself was created using **bookdown**, and acts as an example of what can be achieved. Despite the name containing the word "book", **bookdown** is not only for books, and it can be used for long reports, dissertations, or even single R Markdown documents (see Section \@ref(a-single-document)). It also works with other computing languages such as Python and C++ (see Section \@ref(other-code-languages)). If you want, you can even write documents irrelevant to computing, such as a novel.
...
The `_bookdown.yml` file allows you to specify optional setting used to be build the book. For example, you may want to override the order in which files are merged by including  the field `rmd_files`:
...
The **bookdown** package expands upon the Markdown syntax outlined in Section \@ref(markdown-syntax), and provides additional powerful features that assist longer documents and academic writing.
...
Theorems can be numbered and cross-referenced, as you can see from Theorem \@ref(thm:pyth). The `proof` environment behaves similarly to theorem environments but are unnumbered.
...
A text reference is a paragraph with a label. The sytax is `(ref:label) text`, where `label` is a unique identifier, and `text` is a Markdown paragraph. For example:
...
The **bookdown** package inlucdes the following output formats:
...
To build all Rmd files into a book, you can call the function `bookdown::render_book()`. It uses the settings specified in the `_output.yml` (if exists). If multiple output formats are specified in it, all formats will be built. If you are using RStudio, this can be done through the `Build` tab. Open the drop down menu `Build Book` if you only want to build one format.
...
Building the whole book can be slow when the size of the book is big or your book contains large amounts of computation. We can use the `preview_chapter()` function in **bookdown** to only build a single chapter at time. Equivalently, you can click the `Knit` button in RStudio.
...
You can generate books for both physical and electronic distributions. This section outlines some of the main options.
