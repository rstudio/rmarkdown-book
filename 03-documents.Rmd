...
- `fig_crop` controls whether the `pdfcrop` utility, if available in your system, is automatically applied to PDF figures (this is `true` by default).
    - If you are using [TinyTeX](https://yihui.name/tinytex/) as LaTeX distribution, we recommend that you run `tinytex::tlmgr_install("pdfcrop")` for installing `pdfcrop` (not part of the default installation, see more info in [this **tinytex** issue](https://github.com/yihui/tinytex/issues/118)). You also have to make sure `ghostscript` is available in your system for `pdfcrop` to work.
     - If your graphics device is `postscript`, we recommend that you disable this feature (see more info in [this **knitr** issue](https://github.com/yihui/knitr/issues/1365)).
