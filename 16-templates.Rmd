...
- Customizing the layout of the output document by adding additional fields the YAML metadata. For example, you can add a `department` field to be included within your title page.
...
```{r templates-select, echo=FALSE, fig.cap="Selecting R Markdown templates within RStudio.", out.width='80%', fig.align='center'}
...
You can make use of additional YAML fields from the source document when designing a Pandoc template. For example, you may wish to have a `department` field to be added to your title page, or include an `editor` field to be displayed below the author. We can add additional variables to the Pandoc template by surrounding the variable in dollar signs (`$`) within the template. These variables take values from the YAML metadata of the R Markdown document (or command-line arguments passed the Pandoc). We may also use conditional statements and for-loops. Readers are recommended to check the Pandoc manual for more details: https://pandoc.org/MANUAL.html#using-variables-in-templates.
...
To find out more about packages and the use of GitHub, you may refer to the book "*R Packages*" of [@wickham2015] (http://r-pkgs.had.co.nz/git.html).
