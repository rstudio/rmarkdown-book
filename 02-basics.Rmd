...
It is generally advised that each figure has is provided a caption using `fig.cap`. This will place the R plots into figure environments, which will be automatically labeled and numbered. 
...
```{r hold-position, fig.show='hold', out.width='50%', fig.cap="Side-by-side plots"}
plot(cars, pch = 18)
plot(cars, pch = 19)
```
...
```
````
```{r include-graphics, out.width='25%', fig.align='center', fig.cap='The R Markdown Hex logo'}
