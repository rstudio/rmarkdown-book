...
It is generally advised that each figure has is provided a caption using `fig.cap` in the chunk options. This will place the R plots into figure environments, which will be automatically labeled and numbered. 

If you are outputting to PDF, labelled figures will automatically be placed to try and minimize the amount of whitespace in the document, and may result in figures being placed on a different page to the code chunk used to create it. We recommend not to worry too much about this while creating your documents, but you may wish to fine-tine the positions once the content is complete using the `fig.pos` argument. [This link](https://www.sharelatex.com/learn/Positioning_images_and_tables) provides general tips about this behaviour in LaTeX.

To place multiple figures side-by-side from the same code chunk, we can use the `fig.hold='show'` along with the `out.width` argument, whereby the sum of the `out.width` for the plots must be less than or equal to the current line width. Figure \@ref(fig:hold-position) shows an example with two plots each with a width of `50%`.

```{r hold-position, fig.cap='Side-by-side plots', fig.show='hold', out.width='50%'}
plot(cars, pch = 18)
plot(cars, pch = 19)
```

If you want to include a graphic which is not generated from R code, it is advised to use the `knitr::include_graphics()` function. Compared to the markdown technique of `![alt text or image title](path/to/image)`, the function provides consistent syntax for controlling the image attributes with that of formatting R code. Figure \@ref(fig:include-graphics) provides an example of this.

````markdown
`r ''````{r, out.width='25%', fig.align='center', fig.cap='The R Markdown Hex logo'}
knitr::include_graphics('rmarkdown-min.png')
```
````
```{r include-graphics, fig.cap='The R Markdown Hex logo', out.width='25%', fig.align='center'}
knitr::include_graphics('images/rmarkdown.png')
```

### Tables

The easiest way to include tables is by using `knitr::kable`, which will create tables for HTML, PDF and Word outputs.^[Other packages are available for producing tables within R Markdown, including `xtable`, `Hmisc` and `stargazer`, but are generally less compatable with multiple output formats.] Table captions can be included by passing `caption` to the function:
...
Tables in HTML outputs will always be placed after the code block, but LaTeX output will try and position the table to avoid the table being split within the text. If you want to avoid this behaviour, you will need to use the LaTeX package [longtable](https://www.ctan.org/pkg/longtable) which can break tables across multiple pages. This can be achieved by adding `\usepackage{longtable}` to your LaTeX preamble and passing `longtable = TRUE` to `kable()`.
...
By default, `SELECT` queries will display the first 10 records of their results within the document. The number of records displayed is controlled by the `max.print` option, which is turn derived from the global **knitr** option `sql.max.print` (e.g., `knitr::opts_knit$set(sql.max.print = 10)`; N.B. it is `opts_knit` instead of `opts_chunk`). For example, the following code chunk displays the first 20 records:
