...
If you have options that need to be the result of an evaluated R expression, you should use `!expr`, which tells the 
**yaml** package that it needs to parse that sub-option:

```yaml
output:
  html_document:
    toc: true
    css: !expr system.file("rmd", "h", "bootstrap", "css", "yeti.min.css", package = "rmarkdown")
```
