...
Currently there are two major ways to build multiple Rmd documents: **blogdown** [@xie2017; @R-blogdown] for building websites, and **bookdown** [@xie2016; @R-bookdown] for authoring books. In this chapter, we briefly introduce the **blogdown** package. For the full reference, please read the official **blogdown** book by @xie2017.
...
Once your GIT repository is connected with Netlify, you only need to push source files to the repository in future, and Netlify will automatically rebuild your website. This is called "continuous deployment".
...
The full source code for simple example above can be found in the `hello-website` folder in the repository <https://github.com/rstudio/rmarkdown-website-examples>.
...
In addition to whatever common options you define, the are two output options that are automatically set when rendering a site:
